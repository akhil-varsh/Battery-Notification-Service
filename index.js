...
try {
  ...
  if (serviceAccountPath && !serviceAccountPath.startsWith('MOCK')) {
    ...
    console.warn('Warning: FIREBASE_PRIVATE_KEY_PATH not valid or missing. Notifications will fail unless mocked.');
  }
} catch (error) {
  console.warn(`Warning: Could not load Firebase credentials from ${serviceAccountPath}.`, error.message);
}
...
class BatteryNotificationService {
  ...
  async getStaleLocksFromDynamoDB() {
    ...
    try {
      ...
      console.log(`Found ${result.Items.length} locks with stale battery data`);
      return result.Items;
    } catch (error) {
